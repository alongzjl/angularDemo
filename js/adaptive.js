...
(function (win, verify) {
    ...
    function setViewport() {
        ...
        if (verify.scaleType === 2 && isIPhone || verify.scaleType === 3) {
    ...
    function setRem() {
        ...
        var layoutView;
        if (verify.maxWidth) {
            layoutView = Math.min(docEl.getBoundingClientRect().width, verify.maxWidth * dpr);
        }
        else {
        ...
        newBase = 100 * layoutView / verify.desinWidth * (verify.errDpr || 1);
        docEl.style.fontSize = newBase + 'px';
        // rem基准值改变后，手动reflow一下，避免旋转手机后页面自适应问题
        doc.body&&(doc.body.style.fontSize = verify.baseFont / 100 + 'rem');
        // 重新设置rem后的回调方法
        verify.setRemCallback&&verify.setRemCallback();
        verify.newBase = newBase;
    }
    var tid;
    verify.desinWidth = 750;
    verify.baseFont = 28;
    // 局部刷新的时候部分chrome版本字体过大的问题
    verify.reflow = function() {
        docEl.clientWidth;
    };
    // 检查安卓下rem值是否显示正确
    function checkRem() {
    ...
    verify.init = function () {
        ...
        if (doc.readyState === 'complete') {
            doc.body.style.fontSize = verify.baseFont / 100 + 'rem';
            checkRem();
        }
        else {
    ...
    verify.remToPx = function (remValue) {
